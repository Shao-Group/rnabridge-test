...
if (( $(echo "$x1 >= $y1" | bc -l) )); then
	xx1=`$bin/gtfcuff match-correct $tx $sy $y1 | cut -f 10 -d " "`
	xx2=`$bin/gtfcuff match-correct $tx $sy $y1 | cut -f 16 -d " "`
else
	yy1=`$bin/gtfcuff match-correct $ty $sx $x1 | cut -f 10 -d " "`
	yy2=`$bin/gtfcuff match-correct $ty $sx $x1 | cut -f 16 -d " "`
fi
